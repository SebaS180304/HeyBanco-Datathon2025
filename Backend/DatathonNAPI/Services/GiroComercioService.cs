...
{
    public class GiroComercioService: IGiroComercioService
    {
        private readonly HeyBContext context;
        private readonly IGlobalQueryService globalQueryService;
        public GiroComercioService(HeyBContext heyBContext, IGlobalQueryService globalQueryService_)
        {
            context = heyBContext;
            globalQueryService = globalQueryService_;
        }
        public async Task<Top5Values> TopGiros()
        {
            ...
            var data = await context.BaseTransaccionesFinals.Where(x => ids.Contains(x.Id))
                                                            .GroupBy(x => x.GiroComercio)
                                                            .Select(x=> new { GiroComercio = x.Key, Monto_Total = x.Sum(t=>t.Monto) })
                                                            .ToListAsync();
            float amount = 0;
            ...
            foreach (var Monto in data)
            {
                ...
                if (i < 7)
                {
                    F.values.Add(new Value { name = Monto.GiroComercio ?? "", value = (double)(Monto.Monto_Total ?? 0), percent = (float)(Monto.Monto_Total ?? 0) / amount });
                }
                else if (i > 6)
                {
                    if (F.values.Count() < 8)
